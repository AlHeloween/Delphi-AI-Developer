unit TestAIHarness.View.Main;

interface

uses
  Winapi.Windows, Winapi.Messages, System.SysUtils, System.Variants, System.Classes,
  Vcl.Graphics, Vcl.Controls, Vcl.Forms, Vcl.Dialogs, Vcl.StdCtrls, Vcl.ExtCtrls,
  Vcl.ComCtrls,
  DelphiAIDev.Types, DelphiAIDev.Settings, DelphiAIDev.AI.Facade, DelphiAIDev.AI.Interfaces,
  DelphiAIDev.AI.Response, DelphiAIDev.AI.Gemini;

type
  TFormTestAIHarness = class(TForm)
    pnlTop: TPanel;
    pnlClient: TPanel;
    pnlBottom: TPanel;
    GroupBox1: TGroupBox;
    Label1: TLabel;
    cboAIProvider: TComboBox;
    Label2: TLabel;
    edtApiKey: TEdit;
    btnListModels: TButton;
    btnSend: TButton;
    GroupBox2: TGroupBox;
    mmPrompt: TMemo;
    GroupBox3: TGroupBox;
    mmResponse: TMemo;
    Splitter1: TSplitter;
    pcMain: TPageControl;
    tsSendRequest: TTabSheet;
    tsListModels: TTabSheet;
    Label3: TLabel;
    Label4: TLabel;
    Label5: TLabel;
    tbTemperature: TTrackBar;
    tbTopP: TTrackBar;
    lblTemperatureValue: TLabel;
    lblTopPValue: TLabel;
    edtMaxOutputTokens: TEdit;
    btnSaveSettings: TButton;
    btnLoadSettings: TButton;
    tsLog: TTabSheet;
    mmLog: TMemo;
    procedure FormCreate(Sender: TObject);
    procedure btnSendClick(Sender: TObject);
    procedure btnListModelsClick(Sender: TObject);
    procedure tbTemperatureChange(Sender: TObject);
    procedure tbTopPChange(Sender: TObject);
    procedure btnSaveSettingsClick(Sender: TObject);
    procedure btnLoadSettingsClick(Sender: TObject);
  private
    FSettings: TDelphiAIDevSettings;
    procedure AddToLog(const AText: string);
    procedure LoadSettings;
    procedure SaveSettings;
    procedure SendRequest(const AProvider: TC4DAiAvailable; const AApiKey, AQuestion: string);
    procedure ListModelsForProvider(const AProvider: TC4DAiAvailable; const AApiKey: string);
  public
    { Public declarations }
  end;

var
  FormTestAIHarness: TFormTestAIHarness;

implementation

{$R *.dfm}

const
  SETTINGS_FILE = 'TestAIHarness.ini';

procedure TFormTestAIHarness.FormCreate(Sender: TObject);
var
  LProvider: TC4DAiAvailable;
begin
  FSettings := TDelphiAIDevSettings.Create;

  cboAIProvider.Items.Clear;
  for LProvider := Low(TC4DAiAvailable) to High(TC4DAiAvailable) do
    cboAIProvider.Items.Add(LProvider.ToString);
  cboAIProvider.ItemIndex := 0;

  mmPrompt.Text := 'Write a simple "Hello, World!" program in Delphi.';
  LoadSettings;
  AddToLog('Test Harness initialized.');
end;

procedure TFormTestAIHarness.AddToLog(const AText: string);
begin
  mmLog.Lines.Add(FormatDateTime('[yyyy-mm-dd hh:nn:ss] ', Now) + AText);
end;

procedure TFormTestAIHarness.btnSendClick(Sender: TObject);
begin
  if Trim(edtApiKey.Text) = '' then
  begin
    ShowMessage('Please enter an API Key.');
    Exit;
  end;

  if Trim(mmPrompt.Text) = '' then
  begin
    ShowMessage('Please enter a prompt.');
    Exit;
  end;

  mmResponse.Text := 'Sending request...';
  Application.ProcessMessages;

  SendRequest(
    TC4DAiAvailable(cboAIProvider.ItemIndex),
    edtApiKey.Text,
    mmPrompt.Text
  );
end;

procedure TFormTestAIHarness.btnListModelsClick(Sender: TObject);
begin
  if Trim(edtApiKey.Text) = '' then
  begin
    ShowMessage('Please enter an API Key to list models.');
    Exit;
  end;

  mmResponse.Text := 'Listing models...';
  Application.ProcessMessages;

  ListModelsForProvider(
    TC4DAiAvailable(cboAIProvider.ItemIndex),
    edtApiKey.Text
  );
end;

procedure TFormTestAIHarness.btnSaveSettingsClick(Sender: TObject);
begin
  SaveSettings;
  ShowMessage('Settings saved.');
end;

procedure TFormTestAIHarness.btnLoadSettingsClick(Sender: TObject);
begin
  LoadSettings;
  ShowMessage('Settings loaded.');
end;

procedure TFormTestAIHarness.LoadSettings;
var
  LSettingsFile: string;
begin
  LSettingsFile := ExtractFilePath(ParamStr(0)) + SETTINGS_FILE;
  if FileExists(LSettingsFile) then
  begin
    FSettings.LoadFromFile(LSettingsFile);
    cboAIProvider.ItemIndex := Ord(FSettings.AIDefault);
    case FSettings.AIDefault of
      TC4DAiAvailable.Gemini: edtApiKey.Text := FSettings.ApiKeyGemini;
      TC4DAiAvailable.OpenAI: edtApiKey.Text := FSettings.ApiKeyOpenAI;
      TC4DAiAvailable.Groq: edtApiKey.Text := FSettings.ApiKeyGroq;
      TC4DAiAvailable.Ollama: edtApiKey.Text := FSettings.ApiKeyOllama;
    end;
    tbTemperature.Position := Round(FSettings.Temperature * 10);
    tbTopP.Position := Round(FSettings.TopP * 10);
    edtMaxOutputTokens.Text := IntToStr(FSettings.MaxOutputTokens);
    lblTemperatureValue.Caption := Format('%.2f', [FSettings.Temperature]);
    lblTopPValue.Caption := Format('%.2f', [FSettings.TopP]);
  end
  else
  begin
    edtMaxOutputTokens.Text := '2048';
    tbTemperature.Position := 5;
    tbTopP.Position := 5;
    lblTemperatureValue.Caption := Format('%.2f', [tbTemperature.Position / 10]);
    lblTopPValue.Caption := Format('%.2f', [tbTopP.Position / 10]);
  end;
end;

procedure TFormTestAIHarness.SaveSettings;
var
  LSettingsFile: string;
begin
  LSettingsFile := ExtractFilePath(ParamStr(0)) + SETTINGS_FILE;
  FSettings.AIDefault := TC4DAiAvailable(cboAIProvider.ItemIndex);
  case FSettings.AIDefault of
    TC4DAiAvailable.Gemini: FSettings.ApiKeyGemini := edtApiKey.Text;
    TC4DAiAvailable.OpenAI: FSettings.ApiKeyOpenAI := edtApiKey.Text;
    TC4DAiAvailable.Groq: FSettings.ApiKeyGroq := edtApiKey.Text;
    TC4DAiAvailable.Ollama: FSettings.ApiKeyOllama := edtApiKey.Text;
  end;
  FSettings.Temperature := tbTemperature.Position / 10;
  FSettings.TopP := tbTopP.Position / 10;
  FSettings.MaxOutputTokens := StrToIntDef(edtMaxOutputTokens.Text, 2048);
  FSettings.SaveToFile(LSettingsFile);
end;

procedure TFormTestAIHarness.SendRequest(const AProvider: TC4DAiAvailable;
  const AApiKey, AQuestion: string);
var
  LAIFacade: TDelphiAIDevAIFacade;
  LResponse: IDelphiAIDevAIResponse;
  LSettings: TDelphiAIDevSettings;
  Cont1: TStringList;
  LStatus: string;
begin
  Screen.Cursor := crHourGlass;
  LStatus := 'OK';
  try
    LSettings := TDelphiAIDevSettings.Create;
    try
      case AProvider of
        TC4DAiAvailable.Gemini: LSettings.ApiKeyGemini := AApiKey;
        TC4DAiAvailable.OpenAI: LSettings.ApiKeyOpenAI := AApiKey;
        TC4DAiAvailable.Groq: LSettings.ApiKeyGroq := AApiKey;
        TC4DAiAvailable.Ollama: LSettings.ApiKeyOllama := AApiKey;
      end;
      LSettings.AIDefault := AProvider;
      LSettings.Temperature := tbTemperature.Position / 10;
      LSettings.TopP := tbTopP.Position / 10;
      LSettings.MaxOutputTokens := StrToIntDef(edtMaxOutputTokens.Text, 2048);
      AddToLog(Format('Sending request to %s. Temp: %.2f, TopP: %.2f, MaxTokens: %d',
        [AProvider.ToString, LSettings.Temperature, LSettings.TopP, LSettings.MaxOutputTokens]));

      LAIFacade := TDelphiAIDevAIFacade.Create;
      try
        LResponse := LAIFacade.AiUse(AProvider, LSettings).ProcessSend(AQuestion).Response;
        Cont1 := TStringList.Create;
        try
          Cont1.Assign(LResponse.GetContent);
          mmResponse.Text := Cont1.Text;
          if LResponse.GetStatusCode <> 200 then
          begin
            LStatus := 'FAIL';
            mmResponse.Text := 'Status Code: ' + IntToStr(LResponse.GetStatusCode) + #13#10 + mmResponse.Text;
          end;
          AddToLog(Format('Response received. Status: %s. Status Code: %d.', [LStatus, LResponse.GetStatusCode]));
          AddToLog('--- Response Content ---');
          mmLog.Lines.Add(mmResponse.Text);
          AddToLog('--- End Response ---');
        finally
          Cont1.Free;
        end;
      finally
        LAIFacade.Free;
      end;
    finally
      LSettings.Free;
    end;
  finally
    Screen.Cursor := crDefault;
  end;
end;

procedure TFormTestAIHarness.ListModelsForProvider(const AProvider: TC4DAiAvailable;
  const AApiKey: string);
var
  LAIFacade: TDelphiAIDevAIFacade;
  LModels: string;
begin
  Screen.Cursor := crHourGlass;
  try
    AddToLog(Format('Listing models for %s.', [AProvider.ToString]));
    FSettings.ApiKeyGemini := AApiKey;
    FSettings.AIDefault := AProvider;

    LAIFacade := TDelphiAIDevAIFacade.Create;
    try
      LModels := LAIFacade.AiUse(AProvider, FSettings).ListModels;
      mmResponse.Text := 'Available Models:' + #13#10 + LModels;
      AddToLog('Model list received successfully.');
      AddToLog('--- Models ---');
      mmLog.Lines.Add(LModels);
      AddToLog('--- End Models ---');
    finally
      LAIFacade.Free;
    end;
  finally
    Screen.Cursor := crDefault;
  end;
end;

procedure TFormTestAIHarness.tbTemperatureChange(Sender: TObject);
begin
  lblTemperatureValue.Caption := Format('%.2f', [tbTemperature.Position / 10]);
end;

procedure TFormTestAIHarness.tbTopPChange(Sender: TObject);
begin
  lblTopPValue.Caption := Format('%.2f', [tbTopP.Position / 10]);
end;

end.

// SDID_Metadata {
//   "Framework": "ADID Framework v13.0",
//   "TurnID": 4,
//   "State Vector Manifest": "Add a logging tab to record API requests and responses.
//   "Implement a log tab with a memo to display transaction history."
// }
